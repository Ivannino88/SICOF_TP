...
public class PaqTvBrmRedIPTVIncluidoLI_SN_DAO {

	private static final Logger logger = Logger.getLogger(PaqTvBrmRedIPTVIncluidoLI_SN_DAO.class);
...
//**************** METODO PARA TABLA INICIAL **************************************
	...
	public ServiceResponse getPaqTvBrmRedIPTVIncluidoLI_SN_DAO(String fecha, String todos){
		...
		try {
			...
			if(todos.equals("1"))
			lineasInternasVOList = session.selectList("com.mx.totalplay.SDMapper_Selects.PaqTvBrmRedIPTVIncluidoLI_SNtodos", map);
			else
			lineasInternasVOList = session.selectList("com.mx.totalplay.SDMapper_Selects.PaqTvBrmRedIPTVIncluidoLI_SN", map);

			
			if(lineasInternasVOList.size() >= 0){
				response.setSuccess(true);
				response.setResult(lineasInternasVOList);
				response.setMensaje("Lineas Internas DAO SN Exitoso");
			}else{
				response.setSuccess(false);
				response.setResult(lineasInternasVOList);
				response.setMensaje("Lineas Internas DAO SN NO Exitoso");
			}
			session.close();
		} catch (Exception e) {
...
//****************************************** INSERTAR NUEVOS REGISTROS **************************	
	...
	public ServiceResponse getPaqTvBrmRedIPTVIncluidoLI_insertarSN_DAO(List<LineasInternasVO> listas){
		ServiceResponse response = new ServiceResponse();
		SqlSession session = null;
		Integer result = 0;
		int contador=0;
		...
		try {
			...
			result = session.insert("com.mx.totalplay.SDMapper_Inserts.PaqTvBrmRedIPTVIncluidoLI_insertarSN",map);
...
//********************************* ELIMINAR REGISTROS **********************************************
	public ServiceResponse getPaqTvBrmRedIPTVIncluidoLI_eliminarSN_DAO(List<LineasInternasVO> listas){
		ServiceResponse response = new ServiceResponse();
		SqlSession session = null;
		Integer result = 0;
		try {
			...
			result = session.delete("com.mx.totalplay.SDMapper_Deletes.PaqTvBrmRedIPTVIncluidoLI_eliminarSN",listas);
	...
	public ServiceResponse getEliminarTodoSN_DAO(){
		ServiceResponse response = new ServiceResponse();
		SqlSession session = null;
		Integer result = 0;
		try {
			...
			result = session.delete("com.mx.totalplay.SDMapper_Deletes.PaqTvBrmRedIPTVIncluidoLI_eliminarTodoSN");
...
//******************************************************************************************************
	...
	public ServiceResponse getPaqTvBrmRedIPTVIncluidoLI_Excel(File file){
		...
		try {
			...
			if(excelLibro.size() > 0 && excelLibro.get(0).getSn().equalsIgnoreCase("SN")){	
	    ...
	    while (iterator.hasNext()) {
	        ...
	        while (cellIterator.hasNext()) {
	            ...
	            case 0:
	                ...
	                aBook.setSn(getCellValueStr(nextCell)+"");
	            	else
	            	vacio=true;
	                break;
